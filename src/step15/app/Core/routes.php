...
return function () {
    ...
    $dispatcher = FastRoute\simpleDispatcher(function(FastRoute\RouteCollector $router) {
        ...
        $router->addRoute(['GET'], '/article', function($params) use ($run) {
            $run('ArticleController', 'indexAction', $params);
        });
        $router->addRoute(['GET', 'POST'], '/article/detail', function($params) use ($run) {
            $run('ArticleController', 'detailAction', $params);
        });
        $router->addRoute(['GET'], '/article/rank', function($params) use ($run) {
            $run('ArticleController', 'rankAction', $params);
        });
